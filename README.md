# Enterprise Azure Cloud Security Baseline (Infrastructure as Code)
...
This architecture can be deployed automatically to Microsoft Azure using the provided Bicep template (`main.bicep`). It defines the Virtual Network, Subnets, strict Network Security Groups (NSGs), User Defined Routes (UDRs) for forced tunneling, Azure Bastion secure ingress, and Azure Firewall egress control baseline in a single deployment.
...
3. **Deploy the Bicep template:**
   Deploy to your target Azure Resource Group (e.g. `rg-corporate-security`):
   ```bash
   az deployment group create \
     --name "CorpSecurityBaselineDeployment" \
     --resource-group "rg-corporate-security" \
...
## 🔍 Security Review
This lab includes a post-design vulnerability assessment — identifying weaknesses in the architecture before production deployment.
